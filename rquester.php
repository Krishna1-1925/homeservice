            ...
            <?php
                ...
                if($result->num_rows > 0)
                {
                    ...
                    while($row =$result->fetch_assoc())
                    {
                        ...
                        echo '<input type="hidden" name="id" value='.$row['r_login_id'].'><button type="submit" 
                        class="btn btn-info mr-3 name="edit" value="Edit"><i class="fas fa-pen"></i></button>';
                        echo '</form>';
            
                        echo '<form action=""  method="POST" class="d-inline mt-2" >';
                        echo '<input type="hidden" name="id" value='.$row['r_login_id'].'><button class="btn btn-secondary d-inline" name="delete"     value="Delete"
                ...
                {
                $sql ="DELETE FROM registration WHERE r_login_id = {$_REQUEST['id']}";
