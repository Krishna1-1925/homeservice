...
<title>HomeService-Technician</title>
</head>
<body>
   ...
   <div class="container-fluid">
        ...
        <nav class="col-sm-2 bg-light sidebar py-5 ">
            <div class="sidebar sticky">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item"><a class="nav-link" href="assets.php"><i class="fas fa-list"></i>&nbsp;Assets <ion-icon name="arrow-forward-outline"></ion-icon></a></li>
    ...
    if($result->num_rows > 0){
        ...
        while($row =$result->fetch_assoc()){
            ...
            echo '<input type="hidden" name="id" value='.$row['pid'].'><button type="submit" 
            class="btn btn-info mr-3 name="edit" value="Edit"><i class="fas fa-pen"></i></button>';
            echo '</form>';
            echo '<form action=""  method="POST" class="d-inline mt-2 mr-3" >';
            echo '<input type="hidden" name="id" value='.$row['pid'].'><button class="btn btn-secondary d-inline" name="delete" value="Delete"
            type="submit"><i class="far fa-trash-alt"></i></button>';       
           echo '</form>';
           echo '<form action="sellproduct.php" class="d-inline" method="POST">';
           echo '<input type="hidden" name="id" value='.$row['pid'].'><button type="submit" 
           class="btn btn-warning mr-3 name="issue" value="Issue"><i class="fas fa-handshake"></i></button>';
           echo '</form>';
           echo '</td>';
            echo '</tr>';
        }
    ...
    if(isset($_REQUEST['delete'])){
        $sql ="DELETE FROM assets_id WHERE pid = {$_REQUEST['id']}";
    ...
    <div class="float-right"><a href="addproduct.php" class="btn btn-danger"><i class="fas fa-plus fa-2x"></i></a>
   </div>  
</body>
</html>